// =================================================================
// GUARDA DE SEGURANÇA DA APLICAÇÃO
// =================================================================
...
async function showHistoryModal() {
  try {
    ...
    const response = await fetch('https://controle-de-estoque-une.onrender.com/api/historico', {
        headers: stockManager._getAuthHeaders()
    });
    ...
    const historyRows = historyData.map(log => {
      const formattedDate = new Date(log.data_movimentacao).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
      const badgeColor = log.acao === 'Criação' ? 'bg-success' : log.acao === 'Remoção' ? 'bg-danger' : 'bg-primary';
      return `<tr><td>${formattedDate}</td><td>${log.nome_produto}</td><td><span class="badge ${badgeColor}">${log.acao}</span></td><td class="text-end">${parseFloat(log.quantidade_alterada).toFixed(2).replace('.', ',')}</td><td class="text-end fw-bold">${parseFloat(log.estoque_novo).toFixed(2).replace('.', ',')} ton</td></tr>`;
    }).join('');
    
    historyTableWrapper.innerHTML = `<table class="table table-sm table-striped"><thead class="table-light"><tr><th>Data e Hora</th><th>Produto</th><th>Ação</th><th class="text-end">Alteração (ton)</th><th class="text-end">Estoque Final</th></tr></thead><tbody>${historyRows}</tbody></table>`;
  } catch (error) {
...
function openEditor(productName) {
  if (!productName) {
    ...
    editorName.value = ''; editorPrice.value = ''; editorValue.value = '0';
    editorName.readOnly = false; editorAction.style.display = 'none'; editorValue.parentElement.style.display = 'block';
  } else {
    ...
    editorName.readOnly = true; editorAction.style.display = 'none'; editorValue.parentElement.style.display = 'block';
  }
  editorModal.show();
}
...
async function handleSave() {
  ...
  if (!currentEditingProduct) {
    ...
    try {
      const response = await fetch('https://controle-de-estoque-une.onrender.com/api/produtos', { method: 'POST', headers: stockManager._getAuthHeaders(), body: JSON.stringify(novoProduto) });
    ...
    try {
      const response = await fetch(`https://controle-de-estoque-une.onrender.com/api/produtos/${currentEditingProduct.id}`, { method: 'PUT', headers: stockManager._getAuthHeaders(), body: JSON.stringify(dadosAtualizados) });
